...
To add additional OIDC claims to your SaaS application, configure the following fields for each claim:
...
Cloudflare may include different `response_type` values in the authorization request depending on the selected flow. For example, the implicit flow allows Cloudflare to send the ID token, Access token, or both the ID token and Access token.
